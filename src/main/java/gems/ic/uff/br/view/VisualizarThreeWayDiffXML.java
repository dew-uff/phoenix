 ...
 * @author Leonardo
 */
public class VisualizarThreeWayDiffXML extends JPanel {
    ...
    public VisualizarThreeWayDiffXML(String caminhoOuTextoXML) {
            ...
            public Paint transform(Node arg0) {
                ...
                if (arg0.hasAttributes()) 
                {
                    ...
                    if (nodeSimilaridade != null) 
                    {
                        ...
                        {
                            return Color.WHITE;
                        } else if (nodeSimilaridade.getNodeValue().contains("0.0")) 
                        {
                            
                           
                            //Node nodeSide = arg0.getAttributes().getNamedItem("diff:side");
                           // Object obj = arg0.getUserData("Color");
                            //if(obj != null)
                            //{
...
//                                    (255 * Float.parseFloat(nodeSimilaridade.getNodeValue()))); // similaridade diferente de 0 e 1;
                ...
                else if(arg0.getNodeName().contains("diff:son1")) {
                        return Color.RED;
                }
                else if(arg0.getNodeName().contains("diff:son2")) {
                        return Color.PINK;
                }
                else if(arg0.getNodeName().contains("diff:ancestral")) {
                        return Color.GREEN;
                }
                if (arg0.getNodeType() == Node.TEXT_NODE) {
                    return Color.WHITE;
...
//        treeLayout = new RadialTreeLayout<Node, String>(floresta);
        vv = new VisualizationViewer<Node, String>(treeLayout, new Dimension(1080, 1080));
    ...
    private void insereFilhos(Node item) {
        ...
        for (int i = 0; i < nl.getLength(); i++) 
        {
            if (nl.item(i).getNodeName().contains("diff:value")) 
            {
                ...
                Node noSon1 = nl.item(i).getAttributes().getNamedItem("diff:sron1");
                Node noAncestral = nl.item(i).getAttributes().getNamedItem("diff:ancestral");
                Node noSon2 = nl.item(i).getAttributes().getNamedItem("diff:son2");
                if (noEsquerdo != null) {
                ...
                if (noSon1 != null) {
                    floresta.addEdge(edgeFactory.create(), item, noSon1);
                }
                if (noSon2 != null) {
                    floresta.addEdge(edgeFactory.create(), item, noSon2);
                }
                if (noAncestral != null) {
                    floresta.addEdge(edgeFactory.create(), item, noAncestral);
                }
            } else {
    ...
    private void insereLegendas(JPanel frame){
        ...
        setColorAndTextForSubtitle(containerSubtitles, Color.pink, "Documento 2");
        setColorAndTextForSubtitle(containerSubtitles, Color.green, "Ancestral");
        setColorAndTextForSubtitle(containerSubtitles, Color.yellow, "Ancestral e Documento 1");
        setColorAndTextForSubtitle(containerSubtitles, Color.blue, "Ancestral e Documento 2");
        setColorAndTextForSubtitle(containerSubtitles, Color.orange, "Documento 1 e 2");
